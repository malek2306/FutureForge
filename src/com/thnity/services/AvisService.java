...
public class AvisService {
    ...
    public boolean add(Avis avis) {
        String request = "INSERT INTO `avis`(`id_offre`, `id_etudiant`, `rate`, `description_avis`) VALUES(?, ?, ?, ?)";
        try {
    ...
    public boolean edit(Avis avis) {
        String request = "UPDATE `avis` SET `id_offre` = ?, `id_etudiant` = ?, `rate` = ?, `description_avis` = ? WHERE `id_avis`=" + avis.getId();
        try {
            preparedStatement = connection.prepareStatement(request);
